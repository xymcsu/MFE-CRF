...
def createBIOClass(instances,d_type):
    ...
    #bio_entity.createNER()
    
    bio_entity.createW2VCluster()
    bio_entity.createL2VCluster()
    bio_entity.createP2VCluster()
...
def transformJSONFiles(d_type='re',all_text=False,text_num=200000):
    ...
    if d_type=='re':
        ...
        with open(filepath,'r') as f:
            ...
            if all_text==False:
                review_list=review_list[:text_num]
...
def wordpos_process(text):
    #分词
    words=text.split(' ')
    poss=[w[1] for w in nltk.pos_tag(words)]
    wps=[]
    for i in range(len(words)):
        wps.append(words[i]+poss[i])
    return ' '.join(wps)
    
def transformExtraLemmData(d_type='re'):
    if d_type=='re':
        filepath='../../data/extra/yelp/Restaurants_Raw.csv'
        outpath='../../data/extra/yelp/Restaurants_Lemm.csv'
    else:
        filepath='../../data/extra/amzon/LapTops_Raw.csv'
        outpath='../../data/extra/amzon/LapTops_Lemm.csv'
        
    print('开始获取Lemm.....')
    df=pandas.read_csv(filepath,encoding='gbk')
    df['lemm']=df.text.apply(lemm_process)
...
def transformExtraWordPosData(d_type='re'):
    if d_type=='re':
        filepath='../../data/extra/yelp/Restaurants_Raw.csv'
        outpath='../../data/extra/yelp/Restaurants_WP.csv'
    else:
        filepath='../../data/extra/amzon/LapTops_Raw.csv'
        outpath='../../data/extra/amzon/LapTops_WP.csv'
        
    print('开始获取WordPos.....')
    df=pandas.read_csv(filepath,encoding='gbk')
    df['wordpos']=df.text.apply(wordpos_process)
    df=df.drop(['text'],axis=1)
...
def createDependenceInformation(inputfile,outputfile):
    ...
    print('解析开始')
    eng_parser = StanfordDependencyParser(r"..\..\stanford parser\stanford-parser.jar",r"..\..\stanford parser\stanford-parser-3.6.0-models.jar")
...
def createAllDependence():
    ...
    createDependenceInformation('../../data/ABSA_2014_origin/Restaurants_Test_Data_phaseB.xml','dependences/re_test.dep')
    
    print('获取笔记本数据的句法依赖')
    createDependenceInformation('../../data/ABSA_2014_origin/LapTops_Train_v2.xml','dependences/lp_train.dep')
    createDependenceInformation('../../data/ABSA_2014_origin/LapTops_Test_Data_phaseB.xml','dependences/lp_test.dep')
...
if __name__=='__main__': 
    ...
    #transformExtraLemmData(d_type='re')
    #transformExtraLemmData(d_type='lp')
    
    #transformExtraWordPosData(d_type='re')
    #transformExtraWordPosData(d_type='lp')
