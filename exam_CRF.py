...
def word2features(sent, i,window_size=2):
    ...
    l2v_c=sent[i]['l2v_c']
    l2v_c_c=sent[i]['l2v_c_c']
    p2v_c=sent[i]['p2v_c']
    p2v_c_c=sent[i]['p2v_c_c']
    ...
    for j in range(1,window_size+1):
        ...
        if i+j<=len(sent)-1:
            ...
            #w2v_c1=sent[i+j]['w2v_c']
            #l2v_c1=sent[i+j]['l2v_c']
            #p2v_c1=sent[i+j]['p2v_c']
...
def train_CRF(train_X,train_Y):
    ...
    trainer.params()
    print('开始训练')
    trainer.train('tmp/model/temp.crfsuite')
    print('训练完成')
...
def tag_CRF(test_X):
    ...
    tagger.open('tmp/model/temp.crfsuite')
    print('模型读取完成')
...
def evaluate(detail=False,d_type='re'): 
    ...
    print('加载并处理测试数据集')
    test_corpus=preProcessing.loadXML('../../data/ABSA_2014_origin/%s_Test_Data_phaseB.xml'%d_name)
