...
def wordpos_process(text,poss):
    #分词
    words=text.split(' ')
    wp=[]
    for i in range(len(words)):
        wp.append(words[i]+poss[i])
    return wp
...
def l2v_cluster_processing(lemms,dict_l2v,max_c):
    t_cluster=[]
    for lemm in lemms:      
        try:
            t_cluster.append(dict_l2v[lemm.lower()])
        except:
            t_cluster.append(max_c+1)
            #print('not find WP:%s'%lemm.lower())
    return t_cluster
    
def p2v_cluster_processing(wps,dict_p2v,max_c):
    t_cluster=[]
    for wp in wps:
        try:
            t_cluster.append(dict_p2v[wp.lower()])
        except:
            t_cluster.append(max_c+1)
            #print('not find WP:%s'%wp.lower())
    return t_cluster
...
class BIO_Entity():
    def __init__(self, instances,d_type):
        ...
        #加载斯坦福NER工具
        #self.ner_tool=StanfordNERTagger(model_filename=r'..\..\stanford ner\classifiers\english.all.3class.distsim.crf.ser.gz',path_to_jar=r'..\..\stanford ner\stanford-ner.jar')
    ...
    def createWordPOS(self):
        print('开始构建WordPos序列')
        all_word_pos=[]
        for i in range(len(self.texts)):
            all_word_pos.append(wordpos_process(self.texts[i],self.pos_tags[i]))
        print('构建完成')
        self.word_pos=all_word_pos
    ...
    def createNER(self):
        print('开始NER标记')
        all_texts=[text.split(' ') for text in self.texts]
        all_tags=[]
        for ner in self.ner_tool.tag_sents(all_texts):
            tags=[t[1] for t in ner]
    ...
    def createL2VCluster(self):
        print('开始标记L2V类别')
        dict_L2V=word2vecProcessing.loadDict('cluster/%s_l2v.pkl'%self.d_type)
        max_c=max([item[1] for item in dict_L2V.items()])
        cluster=[l2v_cluster_processing(lemms,dict_L2V,max_c) for lemms in self.lemm_tags]
        print('L2V类别标记完成')
        self.l2v_cluster=cluster
        
        print('开始标记L2V类别_CBOW')
        dict_L2V=word2vecProcessing.loadDict('cluster/%s_l2v_c.pkl'%self.d_type)
        max_c=max([item[1] for item in dict_L2V.items()])
        cluster=[l2v_cluster_processing(lemms,dict_L2V,max_c) for lemms in self.lemm_tags]
        print('L2V类别标记完成_CBOW')
        self.l2v_cluster_c=cluster
        
    def createP2VCluster(self):
        print('开始标记P2V类别')
        dict_P2V=word2vecProcessing.loadDict('cluster/%s_p2v.pkl'%self.d_type)
        max_c=max([item[1] for item in dict_P2V.items()])
        cluster=[p2v_cluster_processing(wps,dict_P2V,max_c) for wps in self.word_pos]
        print('P2V类别标记完成')
        self.p2v_cluster=cluster
        
        print('开始标记P2V类别_CBOW')
        dict_P2V=word2vecProcessing.loadDict('cluster/%s_p2v_c.pkl'%self.d_type)
        max_c=max([item[1] for item in dict_P2V.items()])
        cluster=[p2v_cluster_processing(wps,dict_P2V,max_c) for wps in self.word_pos]
        print('P2V类别标记完成_CBOW')
        self.p2v_cluster_c=cluster
        ...
        for i in range(self.size):
            ...
            for j in range(len(text)):
                feature.append({'word':text[j],'pos':self.pos_tags[i][j],
                                ...
                                'l2v_c':self.l2v_cluster[i][j],
                                'w2v_c_c':self.w2v_cluster_c[i][j],
                                'l2v_c_c':self.l2v_cluster_c[i][j],
                                'p2v_c':self.p2v_cluster[i][j],
                                'p2v_c_c':self.p2v_cluster_c[i][j],
