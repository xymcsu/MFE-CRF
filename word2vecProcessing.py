...
def trainingEmbedding(vector_len=150,d_type='re',add_extra=False,
                      add_word=False,add_Lemm=False,add_POS=False):
    if d_type=='re':
        d_name='Restaurants'
    ...
    train_corpus=preProcessing.loadXML('../../data/ABSA_2014_origin/%s_Train_v2.xml'%d_name)
    test_corpus=preProcessing.loadXML('../../data/ABSA_2014_origin/%s_Test_Data_PhaseA.xml'%d_name)
    print('数据集合并完成')
    ...
    if add_word==True:
        ...
        trainingForWords(texts,vector_len,'embeddingModels/%s.w2v'%d_name,1,0)
        print('创建WordEmbedding_CBOW')
        trainingForWords(texts,vector_len,'embeddingModels/%s.w2v_cbow'%d_name,0,0)
    ...
    if add_Lemm==True:
        ...
        trainingForWords(vectors,vector_len,'embeddingModels/%s.l2v'%d_name,1,0) 
        print('创建LemmEmbedding_CBOW')
        trainingForWords(vectors,vector_len,'embeddingModels/%s.l2v_cbow'%d_name,0,0)
    ...
    if add_POS==True:
        ...
        print('创建WPEmbedding')      
        trainingForWords(vectors,vector_len,'embeddingModels/%s.p2v'%d_name,1,0) 
        print('创建WPEmbedding_CBOW')
        trainingForWords(vectors,vector_len,'embeddingModels/%s.p2v_cbow'%d_name,0,0)
...
def createCluster(cluster_num=10,d_type='re',
    ...
    if add_word==True:      
        print('创建W2V聚类')
        knnClusterForW2V('embeddingModels/%s.w2v'%d_name,'cluster/%s_w2v.pkl'%d_type,cluster_num)
        print('W2V聚类群创建完毕！')
        print('创建W2V_CBOW聚类')
        knnClusterForW2V('embeddingModels/%s.w2v_cbow'%d_name,'cluster/%s_w2v_c.pkl'%d_type,cluster_num)
        print('W2V聚类群创建完毕！')
    
    if add_Lemm==True:
        print('创建L2V聚类')
        knnClusterForW2V('embeddingModels/%s.l2v'%d_name,'cluster/%s_l2v.pkl'%d_type,cluster_num)
        print('L2V聚类群创建完毕！') 
        print('创建L2V_CBOW聚类')
        knnClusterForW2V('embeddingModels/%s.l2v_cbow'%d_name,'cluster/%s_l2v_c.pkl'%d_type,cluster_num)
        print('L2V聚类群创建完毕！')
        
    if add_POS==True:
        print('创建WP2V聚类')
        knnClusterForW2V('embeddingModels/%s.p2v'%d_name,'cluster/%s_p2v.pkl'%d_type,cluster_num)
        print('WP2V聚类群创建完毕！') 
        print('创建WP2V_CBOW聚类')
        knnClusterForW2V('embeddingModels/%s.p2v_cbow'%d_name,'cluster/%s_p2v_c.pkl'%d_type,cluster_num)
        print('WP2V聚类群创建完毕！')
